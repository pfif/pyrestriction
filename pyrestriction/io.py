...
def write_account(account, buf):
    def variable(name, value):
        return "{name} = {value}".format(name=name, value=value)

    def variable_string(name, value):
        return variable(name, '"{value}"'.format(value=value))

    print(variable("REGULAR_INCOME", account.regular_income), file=buf)
    print(variable_string("NAME", account.name), file=buf)
    print(variable_string("CURRENCY", account.currency), file=buf)
    print("", file=buf)
